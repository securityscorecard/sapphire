...
function js (patten, replace, version) {
    return function (filepath) {
        return _util.format( '<script type="text/javascript" src="%s?v=%s"></script>', filepath.replace( patten, replace ), version || '');
    };
}

function css (patten, replace, version) {
    return function (filepath, file, index, length) {
        return _util.format( '<link rel="stylesheet" href="%s?v=%s">', filepath.replace( '/public', '' ), version || '');
    };
}
